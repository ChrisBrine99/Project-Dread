...
#region Additional gamepad functions
...
#region Outline shader functions

/// @description Initializes the code to begin rendering using the outline shader, which will place a 1-pixel
...
function shader_set_outline(_color, _font, _drawOutline = true, _drawCorners = true){
	with(global.shaderOutline){
		...
		if (shader_current() == ID) {return;}
		shader_set(ID);
		
		// Initializes the font to use by doing everything the function "outline_set_font" does, but on a
		// much more condensed scale since there's no need to jump into the shader struct's scope as we are
		// already within it in order to start up the actual shader.
		...
		// Initializes the outline color to use by doing what the function "outline_set_color" does, but on
		// a very condensed scale in comparison. As like above, there's no need to jump into the shader
		// struct's scope since we're already here.
		shader_set_uniform_f_array(sColor, _color);
		curOutlineColor = _color;
		
		// Finally, set the boolean values for the toggles for actually drawing the outlines and/or the corners
		// of the outline right upon start of the shader. The option to disable this is useful for certain
		// aspects like item highlighted and the like, since not all entities will need to have an outline
		// drawn around them in that case--just the entities that are considered "Key Items."
		shader_set_uniform_i(sDrawOutline, _drawOutline);
		shader_set_uniform_i(sDrawCorners, _drawCorners);
	}
}

/// @description Overwrites the current used font in respect to the game's outline shader, but only if that
...
function outline_set_font(_font){
	with(global.shaderOutline){
		...
		// Set the currently used font to the value supplied in the argument space; placing its index into 
		// a variable that keeps track of the current font in use in order to prevent duplicate font setting.
		draw_set_font(_font);
		curFont = _font;
		
		// Grab the font's texture ID from the font texture ID map that was created and fill on the game's
		// initial start up. Then, place the texel dimensions of that page (AKA the size of one pixel on the
		// texture normalized to range between 0 and 1) into the shader's respective uniforms for texel width
		// and height.
		var _fontTexture = global.fontTextures[? _font];
		shader_set_uniform_f(sPixelWidth, texture_get_texel_width(_fontTexture));
		shader_set_uniform_f(sPixelHeight, texture_get_texel_height(_fontTexture));
...
/// @description Overwrites the currently used outline color for the outline shader, but only if the color
...
function outline_set_color(_color){
	with(global.shaderOutline){
		...
		shader_set_uniform_f_array(sColor, _color);
		curOutlineColor = _color; // Store this color in order to check for duplicate color changes later on.
	}
}

#endregion

#region Feathering shader functions

/// @description A simple function that sets the coordinates for the beginning of the feathering effect--which
/// if the values x1, y1, x2, and y2, respectively--and the ending of the feathering effect--which is made
/// up of the remaining four points, respectively--for use with the feathering shader. The ending bounds
/// are what determine the region on the texture where the alpha will be zero, and the starting bounds are
/// what determine the beginning of the feathering from the texture's alpha to that value of zero. Below are
/// what each of the XY pairs correspond to when it comes to the shader's use of them:
///			XY Pair				Function
///				x1, y1		=		Top-left bounds of the no-feathering region
///				x2, y2		=		Bottom-right bounds of that same region
